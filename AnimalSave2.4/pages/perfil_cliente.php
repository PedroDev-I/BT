...
// Atualizar CPF se o formulário for enviado
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cpf'])) {
  ...
  if (!empty($cliente['cpf'])) {
      ...
      // Validar o formato do CPF (verificação simples)
...
<main>
    ...
    <input type="text" name="cpf" id="cpfInput" class="form-control" style="display: none;" value="<?php echo htmlspecialchars($cliente['cpf']); ?>" <?php echo !empty($cliente['cpf']) ? 'readonly' : ''; ?>>
</div>

<div class="mt-2">
    <button id="editarCpfBtn" class="btn btn-warning btn-sm" <?php echo !empty($cliente['cpf']) ? 'style="display: none;"' : ''; ?>>Editar</button>
    <button id="salvarCpfBtn" class="btn btn-success btn-sm" style="display: none;">Salvar</button>
</div>
...
<script>document.getElementById('editarCpfBtn').addEventListener('click', function () {
